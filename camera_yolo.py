    ...
    def yolo_frames(image_hub, unique_name):
        ...
        while True:
            ...
            for track in tracker.tracks:
                ...
                if Camera.intersect(midpoint, previous_midpoint, line[0],
            ...
            cv2.putText(frame, "Total: {} ({} up, {} down)".format(str(total_counter), str(up_count),
                                                                   str(down_count)),
                        (int(0.05 * frame.shape[1]), int(0.1 * frame.shape[0])), 0,
                ...
                if current_minute % write_interval == 0:  # write to file once only every write_interval minutes
                    if current_minute not in count_dict:
                        ...
                        for cls in class_counter:
                            ...
                            class_count_file = open(counts_folder + str(current_date) + '/classes/' + class_filename,
                                                    'a')
                        ...
                        intersection_file = open(
                            counts_folder + str(current_date) + '/intersections/' + intersection_filename, 'a')
